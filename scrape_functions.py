...
def scrape_games(last_game, szn, if_scrape_shifts):
    """
    Scrape a given game
    ...
    pbp_df17 = pd.read_csv("nhl_pbp20162017.csv", dtype=types, encoding='latin-1')
    pbp_df18 = pd.read_csv("nhl_pbp20172018.csv", dtype=types, encoding='latin-1')
    pbp_df1718 = pbp_df17.append(pbp_df18)

    prior_games = max(pbp_df18.Game_Id)

    games = list(range(prior_games+1,last_game+1))
    games = list(map(str, games))
    games = [str(szn) + '0' + x for x in games]
    ...
    pbp_df18.append(pbp_df).to_csv('nhl_pbp20172018.csv', index=False)

    pbp_df = pbp_df1718.append(pbp_df)
    ...
    df_adjustments.xR_Model_Scoring(model_df, pbp_df, '2017_2018')
